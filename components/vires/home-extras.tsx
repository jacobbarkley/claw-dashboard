        ...
        {secondaryPromoted.length > 0 ? secondaryPromoted.map(item => {
          ...
          return (
            <StrategyRow
              ...
              metrics={[
                { label: "Calmar", value: itemPerf.calmar_ratio != null ? itemPerf.calmar_ratio.toFixed(2) : "—", color: "var(--vr-gold)" },
                { label: "vs Bench", value: itemPerf.excess_return_pct != null ? `${itemPerf.excess_return_pct >= 0 ? "+" : ""}${itemPerf.excess_return_pct.toFixed(1)}%` : "—" },
              ]}
