...
import { Delta, fmtCurrency, fmtPct, toneColor, toneOf, type Sleeve } from "./shared"
...
function StrategyUniverse({ universe }: { universe: ViresStrategyUniverse | null }) {
...
export function StocksScreen({ data }: { data: ViresTradingData & { strategy_universe?: ViresStrategyUniverse | null } }) {
  const positions = data.positions.filter(p => (p.asset_type ?? "EQUITY") === "EQUITY")
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <SleeveSummary sleeve="stocks" positions={positions as ViresPosition[]} />
      <OpenPositions positions={positions as ViresPosition[]} />
      <StrategyUniverse universe={data.strategy_universe ?? null} />
    </div>
  )
}
...
export function CryptoScreen({ data }: { data: ViresTradingData }) {
  const positions = data.positions.filter(p => p.asset_type === "CRYPTO")
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <SleeveSummary sleeve="crypto" positions={positions as ViresPosition[]} />
      <OpenPositions positions={positions as ViresPosition[]} />
      <CryptoArchitecture />
    </div>
  )
