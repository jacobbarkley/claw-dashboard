...
export function ViresBenchView({ benchData, operator }: {
  ...
  const [liveBenchData, setLiveBenchData] = useState<BenchData | null>(benchData)
  const [liveOperator, setLiveOperator] = useState<OperatorBundle | null>(operator)

  useEffect(() => {
    setLiveBenchData(benchData)
  }, [benchData])
  ...
  const currentBenchData = liveBenchData ?? benchData
