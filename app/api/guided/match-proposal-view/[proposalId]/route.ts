...
export async function GET(_request: Request, context: { params: Promise<{ proposalId: string }> }) {
  ...
  try {
    const view = await readGuidedMatchProposalView(proposalId)
    return NextResponse.json(view)
  } catch (err) {
    if (err instanceof GuidedUnsafeIdError) {
      return NextResponse.json(
        { error: "invalid_proposal_id", field: err.label, value: err.value, reason: err.message },
        { status: 400 },
      )
    }
    ...
    if (err instanceof GuidedArtifactMissingError) {
      return NextResponse.json(
        { error: "match_proposal_view_not_found", artifact: err.artifactPath },
        { status: 404 },
      )
    }
    if (err instanceof GuidedArtifactInvalidError) {
      return NextResponse.json(
        { error: "match_proposal_view_invalid", artifact: err.artifactPath, reason: err.reason },
        { status: 502 },
      )
    }
    throw err
