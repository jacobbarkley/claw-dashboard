...
export async function GET(_request: Request, context: { params: Promise<{ enrollmentId: string }> }) {
  ...
  try {
    const events = await readEnrollmentEventsView(enrollmentId)
    ...
    if (err instanceof GuidedArtifactMissingError) {
      return NextResponse.json(
        { error: "enrollment_events_not_found", artifact: err.artifactPath },
        { status: 404 },
      )
    }
    if (err instanceof GuidedArtifactInvalidError) {
      return NextResponse.json(
        { error: "enrollment_events_invalid", artifact: err.artifactPath, reason: err.reason },
        { status: 502 },
      )
    }
    throw err
