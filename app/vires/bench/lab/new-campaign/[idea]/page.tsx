...
export default async function ViresLabNewCampaignPage({
  ...
  if (ideaArtifact.strategy_ref.kind !== "REGISTERED") {
    return (
      <>
        <LabSubNav />
        <div style={{ padding: "24px 20px 12px", maxWidth: 640, margin: "0 auto" }}>
          ...
          <h1
            ...
            style={{ margin: 0, fontSize: 26, lineHeight: 1.15, color: "var(--vr-cream)", fontWeight: 400 }}
          >
            No executable strategy yet
          </h1>
          <p style={{ marginTop: 10, fontSize: 12.5, lineHeight: 1.55, color: "var(--vr-cream-mute)" }}>
            This idea is a thesis, not a registered strategy launcher. Draft and register a
            StrategySpec first; once <code style={{ fontFamily: "var(--ff-mono)" }}>strategy_ref.kind</code>{" "}
            becomes <code style={{ fontFamily: "var(--ff-mono)" }}>REGISTERED</code>, the Lab run form unlocks.
          </p>
          <Link
