...
| GUIDED-T1-HARDCODED-SCOPE-REMOVAL | T1/T2 | T1 sub CLOSED 2026-05-09 (T1.0d): `PHASE_6_2_INTERNAL_SCOPE` is gone. User-state reads now require an explicit `GuidedScope` resolved from `lib/guided-scope.server.ts::resolveCurrentScope()`, which derives scope from the verified Auth.js session (email magic link, allowlisted only). Body scope on outgoing commands is now a parity hint; the JWT scope claim minted in `lib/guided-command-auth.server.ts` is authoritative. T2 sub remains open: the email→scope mapping is still single-tenant (one allowlisted email maps to the existing `(jacob, paper_main, default)` scope); generalized multi-tenant scope routing is T2/T3. | T2: scope resolves from authenticated/session context for arbitrary users, not a single-allowlist mapping. | T2 before non-Jacob users. | T1 closure: hardcoded constant removed (PR for T1.0d); preview pages redirect to `/signin` when unauthenticated and render labeled mock fallback when the authenticated identity has no scope mapping. T2 closure: multi-tenant scope routing replaces the single-email mapping; tests cover unsafe/mismatched scope paths. |
