...
| GUIDED-T1-HARDCODED-SCOPE-REMOVAL | T1/T2 | **CLOSED 2026-05-09 (T1 sub).** `PHASE_6_2_INTERNAL_SCOPE` hardcodes `(jacob, paper_main, default)` for user-state reads. | Scope resolves from authenticated/session context or explicit internal dev configuration, not a permanent constant. | **T1 sub closed by T1.0d** (claw-dashboard PR #11): constant deleted; user-state read helpers no longer have a default scope param; the 5 user-state preview pages call `resolveCurrentScope()` and pass scope explicitly; unauthenticated routes redirect to `/signin`. T2 sub remains open until non-Jacob users are admitted. | `PHASE_6_2_INTERNAL_SCOPE` deleted from `lib/guided-data-source.server.ts`; `lib/guided-scope.server.ts:resolveCurrentScope()` derives scope from the authenticated session; `tsc` would fail on any caller without an explicit scope. T2 sub: pending non-Jacob admission. |
