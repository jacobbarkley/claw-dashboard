...
| GUIDED-T1-QUESTIONNAIRE-CAPITAL-CADENCE | T1 | The Visual Walk identified a funding/capital-cadence question whose answers did not discriminate matching. The current committed snapshot appears to have removed it; reintroduction must be meaningful. | Funding/capital-cadence is dropped, reframed, or conditionally shown only when it changes allocation/matching behavior. | Before any funding/capital-cadence question is shown as a real matcher input. | Questionnaire JSON and matcher semantics agree; tests prove the answer either affects matching/allocation or is not collected. |
...
| LAB-TQ-PRIVATE-BY-DEFAULT-CONTRIBUTION | T_Q / Lab Phase 0 | Private-by-default principle is captured, but contribution/license flow is not built. | User-created strategies remain private unless a deliberate contribution/license flow grants broader use. | Before user-created Lab strategy can be referenced by Talon, matching, marketing, aggregation, or other users. | Contribution contract; consent UI; attribution/provenance ledger; anonymization standard documented with thresholds/parameters and reviewed by Jacob before aggregates. |
